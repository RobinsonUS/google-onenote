...
function moveY(px: number, feetY: number, pz: number, dy: number, velY: number, world: WorldData): { feetY: number; velY: number; onGround: boolean } {
  ...
  for (let i = 0; i < 10; i++) {
    const mid = (lo + hi) / 2;
    if (aabbOverlaps(px, feetY + sign * mid, pz, world)) hi = mid;
    else lo = mid;
  }
  const safeFeetY = feetY + sign * lo;
  if (dy <= 0) {
    const snapped = Math.ceil(safeFeetY - 0.001);
    if (!aabbOverlaps(px, snapped, pz, world)) return { feetY: snapped, velY: 0, onGround: true };
...
interface CamState { yaw: number; pitch: number; }

function CameraController({ moveRef, camRef, worldRef, onCamPos, cameraRef }: {
  ...
  useEffect(() => {
    ...
    const onJump = () => { if (onGround.current) { velY.current = 9.5; onGround.current = false; } };
  ...
  useFrame((_, delta) => {
    ...
    if (!aabbOverlaps(newX, feetY, pos.z, world)) pos.x = newX;
    const newZ = pos.z + moveZ * speed * dt;
    if (!aabbOverlaps(pos.x, feetY, newZ, world)) pos.z = newZ;
    pos.y = feetY + EYE;
    onCamPos(pos);
...
function screenRaycast(screenX: number, screenY: number, camera: THREE.Camera, world: WorldData, maxDist = 6) {
...
function raycastBlock(origin: THREE.Vector3, yaw: number, pitch: number, world: WorldData, maxDist = 6) {
...
function blockOverlapsPlayer(bx: number, by: number, bz: number, playerPos: THREE.Vector3) {
  const feetY = playerPos.y - EYE;
  const headY = playerPos.y + HEAD;
  return (playerPos.x + HALF > bx && playerPos.x - HALF < bx + 1 && headY > by && feetY < by + 1 && playerPos.z + HALF > bz && playerPos.z - HALF < bz + 1);
}
...
export function MinecraftGame() {
  ...
  const lookTouchRef = useRef<{ id: number; startX: number; startY: number; startYaw: number; startPitch: number; startTime: number; } | null>(null);
  const miningRef = useRef<{ active: boolean; targetKey: string; blockType: number; elapsed: number; lastTime: number; screenX?: number; screenY?: number; } | null>(null);
  ...
  const startMining = useCallback((screenX?: number, screenY?: number) => {
    ...
    if (!result?.hit) { stopMining(); return; }
    const key = posKey(result.x, result.y, result.z);
    const mining = miningRef.current;
    if (mining && mining.active && mining.targetKey === key) {
      if (screenX !== undefined && screenY !== undefined) { mining.screenX = screenX; mining.screenY = screenY; setMiningPos({ x: screenX, y: screenY }); }
      return;
    }
    miningRef.current = { active: true, targetKey: key, blockType: result.blockType, elapsed: 0, lastTime: performance.now(), screenX, screenY };
    setMiningActive(true); setMiningProgress(0);
    setMiningPos(screenX !== undefined && screenY !== undefined ? { x: screenX, y: screenY } : undefined);
    if (!miningRafRef.current) miningRafRef.current = requestAnimationFrame(miningLoop);
  }, []);

  const stopMining = useCallback(() => {
    miningRef.current = null; setMiningActive(false); setMiningProgress(0);
    if (miningRafRef.current) { cancelAnimationFrame(miningRafRef.current); miningRafRef.current = null; }
  }, []);

  const miningLoop = useCallback(() => {
    const mining = miningRef.current;
    if (!mining || !mining.active) { miningRafRef.current = null; setMiningActive(false); setMiningProgress(0); return; }
    ...
    if (currentKey !== mining.targetKey) {
      miningRef.current = null; miningRafRef.current = null;
      if (isHoldingRef.current) startMining(mining.screenX, mining.screenY);
      else { setMiningActive(false); setMiningProgress(0); }
      return;
    }
    const now = performance.now();
    const dt = (now - mining.lastTime) / 1000;
    mining.lastTime = now; mining.elapsed += dt;
    ...
    if (progress >= 1) {
      ...
      if (isHoldingRef.current) {
        setTimeout(() => { if (isHoldingRef.current) startMining(holdTouchPosRef.current.x, holdTouchPosRef.current.y); }, 50);
      } else setMiningActive(false);
      miningRafRef.current = null; return;
    }
    miningRafRef.current = requestAnimationFrame(miningLoop);
  }, [emitParticles, mutateWorld]);

  useEffect(() => {
    if (miningRef.current?.active && !miningRafRef.current) miningRafRef.current = requestAnimationFrame(miningLoop);
  }, [miningLoop]);
  ...
  const placeBlock = useCallback((screenX?: number, screenY?: number) => {
    ...
    if (result?.hit) {
      if (result.blockType === BLOCK_TYPES.CRAFTING_TABLE) { setCraftingTableOpen(true); return; }
  ...
  const handleLookTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      ...
      if (touch.clientX > window.innerWidth * 0.35 && lookTouchRef.current === null) {
        lookTouchRef.current = { id: touch.identifier, startX: touch.clientX, startY: touch.clientY, startYaw: camRef.current.yaw, startPitch: camRef.current.pitch, startTime: Date.now() };
        holdTouchPosRef.current = { x: touch.clientX, y: touch.clientY };
        isHoldingRef.current = true;
        setTimeout(() => { if (isHoldingRef.current && lookTouchRef.current) startMining(holdTouchPosRef.current.x, holdTouchPosRef.current.y); }, 1000);
      }
    }
  }, [startMining]);
  ...
  const handleLookTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      ...
      if (lookTouchRef.current && touch.identifier === lookTouchRef.current.id) {
        ...
        camRef.current.pitch = Math.max(-1.4, Math.min(1.4, lookTouchRef.current.startPitch + dy * -0.005));
        if (miningRef.current?.active) {
          miningRef.current.screenX = touch.clientX; miningRef.current.screenY = touch.clientY;
  ...
  const handleLookTouchEnd = useCallback((e: React.TouchEvent) => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      const touch = e.changedTouches[i];
      if (lookTouchRef.current && touch.identifier === lookTouchRef.current.id) {
        isHoldingRef.current = false; stopMining();
        ...
        if (!moved && elapsed < 1000 && !craftingTableOpen) {
          ...
          if (tapResult?.hit && tapResult.blockType === BLOCK_TYPES.CRAFTING_TABLE) setCraftingTableOpen(true);
          else placeBlock(lookTouchRef.current.startX, lookTouchRef.current.startY);
        }
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space' && e.type === 'keydown') window.dispatchEvent(new CustomEvent('mc-jump'));
      if (e.type === 'keydown') { const num = parseInt(e.key); if (num >= 1 && num <= 9) setSelectedIndex(num - 1); }
    };
    ...
    const onClick = () => { if (document.pointerLockElement !== document.body) document.body.requestPointerLock(); };
    ...
    document.addEventListener('mousemove', mouseLook); document.addEventListener('click', onClick);
    return () => { clearInterval(frame); document.removeEventListener('keydown', onKey); document.removeEventListener('keyup', onKey); document.removeEventListener('mousemove', mouseLook); document.removeEventListener('click', onClick); };
  }, []);
  ...
  useEffect(() => {
    const onMouseDown = (e: MouseEvent) => {
      ...
      if (e.button === 0) { desktopMiningRef.current = true; isHoldingRef.current = true; startMining(); }
      if (e.button === 2) { e.preventDefault(); placeBlock(); }
    };
    const onMouseUp = (e: MouseEvent) => { if (e.button === 0) { desktopMiningRef.current = false; isHoldingRef.current = false; stopMining(); } };
    const onContextMenu = (e: MouseEvent) => e.preventDefault();
    document.addEventListener('mousedown', onMouseDown); document.addEventListener('mouseup', onMouseUp); document.addEventListener('contextmenu', onContextMenu);
    return () => { document.removeEventListener('mousedown', onMouseDown); document.removeEventListener('mouseup', onMouseUp); document.removeEventListener('contextmenu', onContextMenu); };
  }, [startMining, stopMining, placeBlock]);

  const handleJoystickMove = useCallback((state: { dx: number; dz: number }) => { moveRef.current = state; }, []);
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', position:'relative', overflow:'hidden', touchAction:'none' }} onContextMenu={(e) => e.preventDefault()}>
      ...
      <div style={{ position:'fixed', bottom:100, right:20, zIndex:100 }}>
        <div style={{ width: 72, height: 72, background: 'rgba(255,255,255,0.13)', border: '2px solid rgba(255,255,255,0.25)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', touchAction: 'none' }}
          onTouchStart={(e) => { e.stopPropagation(); window.dispatchEvent(new CustomEvent('mc-jump')); }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15" /></svg>
        </div>
      </div>
      ...
      {inventoryOpen && <InventoryScreen inventory={inventory} onInventoryChange={setInventory} onClose={() => setInventoryOpen(false)} selectedHotbarIndex={selectedIndex} />}
      {craftingTableOpen && <CraftingTableScreen inventory={inventory} onInventoryChange={setInventory} onClose={() => setCraftingTableOpen(false)} selectedHotbarIndex={selectedIndex} />}
    </div>
  );
}
